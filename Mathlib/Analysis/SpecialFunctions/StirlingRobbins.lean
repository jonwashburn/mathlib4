...
import Mathlib.Analysis.SpecialFunctions.Gamma.BinetFormula

/-!
# Robbins bounds for Stirling's approximation

This file proves Robbins' sharp two-sided bounds for the factorial, for `n ≥ 1`:

`Real.sqrt (2 * Real.pi * n) * (n / Real.exp 1) ^ n * Real.exp (1 / (12 * n + 1)) ≤ n!`

and

`n! ≤ Real.sqrt (2 * Real.pi * n) * (n / Real.exp 1) ^ n * Real.exp (1 / (12 * n))`.

These bounds are derived from the real Binet formula
(see `Mathlib/Analysis/SpecialFunctions/Gamma/BinetFormula.lean`)
and bounds for the Binet correction term `Binet.J`.

## Main results

* `Stirling.log_factorial_theta`
* `Stirling.factorial_upper_robbins`
* `Stirling.factorial_lower_robbins`
* `Stirling.stirling_asymptotic`
...
theorem log_factorial_theta {n : ℕ} (hn : 0 < n) :
  ...
  have h_fact : Real.log (n.factorial) = Real.log n + Real.log (Real.Gamma n) := by
    rw [← Real.log_mul (Nat.cast_ne_zero.mpr (ne_of_gt hn)) (Real.Gamma_pos_of_pos
  ...
  have h_binet :
    ...
    exact Binet.Real_log_Gamma_eq_Binet hx
  ...
  rw [h_theta]
  have h_log_term : Real.log (2 * Real.pi * n) / 2 = Real.log (2 * Real.pi) / 2 +
      Real.log n / 2 := by
...
This is Robbins' sharp upper bound for Stirling's approximation. -/
...
This refined lower bound on the Binet integral uses monotonicity of `Ktilde`. -/
lemma J_lower_bound (n : ℕ) :
  ...
  have h_bound : ∀ t ∈ Ioi 0, (1 / 12) * Real.exp (-t / 12) ≤ BinetKernel.Ktilde t := by
    intro t ht
    simpa using (BinetKernel.Ktilde_ge_one_div_twelve_mul_exp_neg_div_twelve (t := t)
      (by simpa using ht))
  have h_int_le : ∫ t in Ioi 0, (1 / 12) * Real.exp (-t / 12) * Real.exp (-t * x) ≤
    ...
    exact h_bound t ht
  have h_lhs :
      ∫ t in Ioi 0, (1 / 12) * Real.exp (-t / 12) * Real.exp (-t * x) = 1 / (12 * x + 1) := by
    have hx' : 0 < x + 1 / 12 := by linarith [hx]
    have hexp : ∀ t : ℝ,
        (1 / 12 : ℝ) * Real.exp (-t / 12) * Real.exp (-t * x)
    ...
    calc
      ...
      _ = (1 / 12 : ℝ) * (1 / (x + 1 / 12)) := by
              simpa using congrArg (fun y : ℝ => (1 / 12 : ℝ) * y) hbase
      _ = 1 / (12 * x + 1) := by
              have hx12 : (x + (1 / 12 : ℝ)) ≠ 0 := ne_of_gt hx'
              field_simp [hx12]
...
theorem factorial_lower_robbins (n : ℕ) (hn : 0 < n) :
  ...
  have h_log_ge : n * Real.log n - n + Real.log (2 * Real.pi * n) / 2 + 1 / (12 * n + 1) ≤
    ...
    have h_fact : Real.log (n.factorial) = Real.log n + Real.log (Real.Gamma n) := by
      ...
      rw [← Real.Gamma_add_one (Nat.cast_ne_zero.mpr (ne_of_gt hn))]
      rw [Real.Gamma_nat_eq_factorial]
    have h_binet : Real.log (Real.Gamma n) = (n - 1 / 2) * Real.log n - n + Real.log
        (2 * Real.pi) / 2 + (Binet.J n).re := by
      exact Binet.Real_log_Gamma_eq_Binet hn_pos
    rw [h_fact, h_binet]
  ...
  rw [Real.log_mul (mul_pos h_sqrt_pos (pow_pos (div_pos hn_pos (Real.exp_pos 1)) n)).ne'
    (Real.exp_pos _).ne']
  rw [Real.log_exp]
...
theorem factorial_asymptotic :
  ...
  have h_squeeze : ∀ᶠ n in atTop, lower n ≤ (n.factorial : ℝ) / (Real.sqrt (2 * Real.pi * n) *
      (n / Real.exp 1) ^ n) ∧ (n.factorial : ℝ) / (Real.sqrt (2 * Real.pi * n) *
      (n / Real.exp 1) ^ n) ≤ upper n := by
    ...
    have h_log_stirling : Real.log stirling = n * Real.log n - n +
        Real.log (2 * Real.pi * n) / 2 := by
      have h2pi_pos : (0 : ℝ) < 2 * Real.pi := by nlinarith [Real.pi_pos]
      have hn_pos : (0 : ℝ) < n := Nat.cast_pos.mpr hn
    ...
    · simp only [upper]
      ...
      simpa [one_div, c, add_assoc, add_left_comm, add_comm, mul_assoc, mul_left_comm, mul_comm]
        using hc
