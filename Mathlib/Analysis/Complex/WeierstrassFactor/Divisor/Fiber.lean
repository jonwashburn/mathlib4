...
-/
import Mathlib.Analysis.Complex.WeierstrassFactor.Divisor.Convergence
import Mathlib.Analysis.Complex.WeierstrassFactor.Divisor.Units
...
lemma divisor_univ_eq_analyticOrderNatAt_int {f : ℂ → ℂ} (hf : Differentiable ℂ f) (z : ℂ) :
  ...
  | coe n => simp [han.meromorphicOrderAt_eq, h]
...
theorem divisorZeroIndex₀_fiber_finite (f : ℂ → ℂ) (z₀ : ℂ) :
  ...
  have hfin :
      ({p : divisorZeroIndex₀ f (Set.univ : Set ℂ) | ‖divisorZeroIndex₀_val p‖ ≤ ‖z₀‖} :
        Set _).Finite := by
    simpa using
      (divisorZeroIndex₀_norm_le_finite (f := f) (U := (Set.univ : Set ℂ)) (B := ‖z₀‖)
        (by simp))
  refine hfin.subset ?_
  intro p hp
  have hp' : divisorZeroIndex₀_val p = z₀ := by
...
theorem eventually_atTop_subset_fiberFinset
    (f : ℂ → ℂ) (z₀ : ℂ) :
    ∀ᶠ s : Finset (divisorZeroIndex₀ f (Set.univ : Set ℂ)) in
        (Filter.atTop : Filter (Finset (divisorZeroIndex₀ f (Set.univ : Set ℂ)))),
      divisorZeroIndex₀_fiberFinset (f := f) z₀ ⊆ s := by
  exact Filter.eventually_atTop.2 ⟨divisorZeroIndex₀_fiberFinset (f := f) z₀, fun _ hs => hs⟩
...
lemma divisorZeroIndex₀_fiberFinset_card_eq_toNat_divisor (f : ℂ → ℂ) {z₀ : ℂ} (hz₀ : z₀ ≠ 0) :
  ...
  have hcard : Nat.card S = n := by
    -- `S` is the fiber over `z₀`, hence equivalent to `Fin n`.
    ...
    let e : S ≃ Fin n :=
      { toFun := fun x => by
        ...
        left_inv := by
          ...
          rfl
        right_inv := fun _ => rfl }
    simpa using (Nat.card_congr e).trans (by simp [n])
  ...
  have htoFinset : S.ncard = (divisorZeroIndex₀_fiberFinset (f := f) z₀).card := by
    simpa [divisorZeroIndex₀_fiberFinset, S] using
      (Set.ncard_eq_toFinset_card S (divisorZeroIndex₀_fiber_finite (f := f) z₀))
