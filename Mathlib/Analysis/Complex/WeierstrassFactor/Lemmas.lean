...
/-!
# Power bounds for Weierstrass elementary factors

This file proves quantitative estimates for the Weierstrass elementary factors
`E_m(z) = (1 - z) * exp (∑_{k=1}^m z^k / k)` (implemented as `Complex.weierstrassFactor`).
...
We also record auxiliary lower bounds for `Real.log ‖weierstrassFactor m z‖`.
...
lemma log_norm_weierstrassFactor_ge_neg_two_pow {m : ℕ} {z : ℂ} (hz : ‖z‖ ≤ (1 / 2 : ℝ)) :
  ...
  have hEq : weierstrassFactor m z = Complex.exp (-logTail m z) :=
    weierstrassFactor_eq_exp_neg_tail m hz_lt
  have hlog : Real.log ‖weierstrassFactor m z‖ = (-logTail m z).re := by
...
lemma log_norm_weierstrassFactor_ge_log_norm_one_sub_sub (m : ℕ) (z : ℂ) :
  ...
  have hlog : Real.log ‖weierstrassFactor m z‖ = Real.log ‖1 - z‖ + S.re := by
